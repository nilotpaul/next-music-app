  ...
  return (
    ...
    >
      {songs.map((song, id) => (
        <Card
          key={song.id}
          className="group relative flex min-h-[12rem] min-w-[7.75rem] cursor-pointer flex-col rounded-[0.4rem] border-none bg-muted transition-colors duration-300 hover:bg-muted hover:transition-colors hover:duration-300 xs:min-h-[15rem] xs:min-w-[9.375rem] md:max-h-[15rem] md:min-h-[16.25rem] md:min-w-[11.3rem] md:max-w-[10rem] md:bg-popover/70"
        >
          ...
          <CardFooter className="mx-4 flex flex-col items-start gap-y-1 px-0 xs:mb-3">
            <span className="truncate text-sm font-semibold xs:text-base">
              {song.title}
            </span>
            <span className="truncate text-xs text-neutral-400/80 xs:text-sm">
              {song.artistName}
            </span>
            <PlayPauseButton
              queueName={queueName || ""}
